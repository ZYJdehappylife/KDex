...
function newAPIForm(baseUrl: string) {
  return {
    ...
    name: 'KDex API upstream',
    type: CHANNEL_TYPE_KDEX_API,
    base_url: baseUrl,
    key: 'test-key',
    models: 'gpt-5',
  }
...
describe('KDex API channel', () => {
  test('registers selection, ordering, model discovery, and icon metadata', () => {
    ...
    expect(MODEL_FETCHABLE_TYPES.has(CHANNEL_TYPE_KDEX_API)).toBe(true)
    expect(getChannelTypeIcon(CHANNEL_TYPE_KDEX_API)).toBe('NewAPI')
    expect(getKeyPromptForType(CHANNEL_TYPE_KDEX_API)).toBe(
      'Enter API key for this channel'
    )
    expect(getChannelTypeConfig(CHANNEL_TYPE_KDEX_API).icon).toBe('NewAPI')
  })
  ...
  test('requires a non-blank Base URL', () => {
    ...
    expect(
      channelFormSchema.safeParse(newAPIForm('https://kdex-api.example')).success
    ).toBe(true)
  })
