...
export const channelFormSchema = z
  ...
  .superRefine((data, ctx) => {
    if (
      [3, 8, 36, 45, CHANNEL_TYPE_KDEX_API].includes(data.type) &&
      !data.base_url?.trim()
    ) {
      addRequiredIssue(
        ctx,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  // Field passthrough controls:
  // - OpenAI, Anthropic, Codex, and KDex API: allow_service_tier
  // - OpenAI request fields: OpenAI, Codex, and KDex API
  // - Claude request fields: Anthropic and KDex API
