...
// are exposed separately for self-service flows driven from kdex-api's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("kdex-api-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "kdex-api-store"
	defaultWaffoPancakeProductName = "kdex-api-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because kdex-api has no renewal-
// event handling; Pancake auto-renewing without kdex-api extending user
