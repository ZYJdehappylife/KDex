...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/KDexAPI/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/KDexAPI/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/KDexAPI/models.json", base), fmt.Sprintf("%s/api/KDexAPI/vendors.json", base)
}
